...
## 実装時の注記（採択後に追記、2026-09-04）

- **提案者 C を Ministral 3 8B から Gemma 4 12B-it に替えた。** 同じ Task を 5 ラウンド回したところ、
  Ministral は 3 回、文脈行を重複させた diff（`-` 行と同内容の文脈行を並べる）を返し `git apply` に
  失敗した。Vulkan の MMVQ カーネルの数値バグ（llama.cpp Issue #27007、Q4_K 全般に及ぶ）を回避した
  状態でも 3 回中 3 回再現したので、モデル由来と判断した。Gemma 4 12B-it（Google、Apache-2.0、
  LiveCodeBench v6 72.0）は同じ回避策のもとで 3 ラウンド連続で正しい diff を返し verifier を通過した。
  系統は Google で、IBM・Alibaba のどちらとも重ならない（D2 の異種性は保たれる）。
- 3 体とも thinking は無効にして運用する。Granite で思考予算を 128 トークンに絞る A/B を行ったところ、
  予算切れの後も content 内で考え続けて最初に出した誤った diff が抽出され、`apply_failed` になった。
  思考なし（`-rea off`）では正しい diff を返した。ADR-0004 の「思考タグは信用せず除去する」に加え、
  コーディング面 v0 では思考そのものを使わない。
- 上の 2 点はプールの**中身**の変更であり、D1〜D8 の規則（設定順・1 サンプル・JSON 設定・
  Byzantine 許容数の記録）は変えていない。モデル名は `local/` の設定にあり、本記録の表は
  採択時点の選定として残す。
