...
- **審判の隔離にハードウェアの分離は要らない**。Self-Harness（2606.09498）も AHE（2604.25850）も
  ...
  その対策は**別のモデル系列**である。所有者の参照機ではローカル iGPU 上の審判が NPU 上より速く、
  同時常駐でき、量子化も元の形式のまま使える。NPU は平行して試し、依存にはしない。
...
**iGPU**（NPU は平行試験）。人間ラベルは**公開データで種を撒き、所有者がブラウザのページで少量**。
...
`candidates_origin: external` と各ファイルの sha256 を記録する。`--seed` は提示順と nonce だけを動かし、
...
- **盲検**。審判は提案者 id、モデル名、応答長、応答時間を受け取らない。候補は `A` / `B` と呼ばれ、
  候補と提案者の対応はトレースにだけある。提示順の置換は run-id から導いた PRNG で決め、記録する。
- **6 コール**。3 ペア × 2 順序。ペアは**両順序で同じ候補を選んだときだけ**その候補の勝ち。不一致、
  どちらかが `tie`、どちらかが不正出力なら**引き分け**（片方が不正だったから勝ち、は無い）。
  2 勝の候補が一意なら `Selected`（Condorcet 勝者）。それ以外は `NoCandidate` にサブ理由
...
- **記録**。`judge/<i>-<ab|ba>.json` に送ったメッセージ列と生の応答（API キーは除く）、`judge.json` に
  置換・nonce・ペアごとの両順序の結果と一致・勝ち数・結末・再送回数・サニタイズ・フラグ・遅延・
  候補の文体量（トークン数、見出し・箇条書き・強調・コードフェンスの数）。審判が見たものは全部
  再構成できる。
