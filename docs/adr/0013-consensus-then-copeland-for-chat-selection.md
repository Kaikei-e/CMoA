...
- 小文字化。
- markdown の強調記号 `*` と `` ` `` を除去。**アンダースコアは落とさない**——語の構成文字であり、
  落とせば `user_id` と `userid` が同じ答えになる。
- 箇条書き記号と番号を、**行頭に限らず項目の始まりすべてで**除去する。1 行に書かれた列挙
  （`1. りんご 2. みかん 3. ぶどう`）で先頭の 1 つしか落とさないと、末尾の数が 3 になり、3 で終わる短い
  答えと何でも一致してしまう。マーカーと見なすのは、行頭または空白の直後にあり、**後ろが数字でも空でも
  ないとき**だけである（`1. 答え` は項目、`101.` は答え、`1.101` は小数）。
- 空白の連続を 1 つに畳み、両端を落とす。
- 末尾の `.` `。` `!` `?` を落とす。

2 つの答えが**一致する**のは、(i) 正規化した文字列が等しいとき、または (ii) 次を**すべて**満たすとき：
両方が 160 runes 以下、両方に数が含まれる、末尾の数が正準形で等しい、**そして両方が否定を含むか、
両方が含まないか**。末尾の数を見るのは、筆算は被演算子を先に、結果を最後に書くからである。長さの上限は、
数を 1 つ共有するだけの長文どうしを一致と呼ばないためのものである。どちらの検査も質問も
`reference.answer` も読まない——共有された読み違いで一致に持ち込まれないようにするためである。

数値一致には、読みが甘くならないための安全弁を 3 つ置く。

- **否定の一致を要求する。** レビューで見つかった失敗は `3つあります` と `3つではありません` で、
  どちらも短く、末尾の数が同じで、この安全弁が無ければ**審判コール 0 で同じ答えとして返っていた**。
  否定形の小さな一覧を照合し（日本語は部分一致、英語は語境界付きで、`know` の中の `no` では発火しない）、
  両者の判定が一致することを要求する。一覧は取りこぼす。取りこぼしも誤検出も**一致を失うだけ**で、
  どちらの誤りも「審判に訊く」に落ちる——正しさを買うために合意率を捨てる、意図した非対称である。
- **桁区切りには形がある。** `,` と `、` が桁区切りなのは、直前が数字で直後がちょうど 3 桁、その次が
  数字でないときだけである。`、` は日本語の読点でもあり、`候補は 1、2、3 です` は 123 ではない。
- **指数は 1 つのトークンとして原文のまま扱う。** `1e5` は `5` でも `100000` でもない。取りこぼしは
  安全だが、偽の一致は安全ではない。
...
**機械の故障はスコアより上位に残る。** 0011 D4 の escalate は残すが、**問いを結末が決まる通貨で訊く**。
スコアの下では、答えの無いペアは全勝を作らなくても効く——0.5 点で首位を追い越せるし、連鎖が解く同点集合に
入ることもできる。したがって、単独首位以外の候補が、未回答のペアが全部その候補に転んだとき
**最高スコアに届き得る**なら `judge_timeout` / `judge_failed`（timeout が優先）。単独首位自身は数えない
——点が増えても首位が確認されるだけで、追い付き得る相手はその相手の行で検査される。解析できないペアが
あれば `no_candidate` / `invalid_output`。比べる答えが 2 つ未満なら `too_few_candidates`。

### D4. 同点は鍵の連鎖で解き、効いた鍵を——どの鍵も決めなかったことも含めて——記録する
...
2. **短い方（ゲート付き）**（`length`）——正規化した答えが**一意に最短**で、かつ**最長が最短の
   ...
   1.5 を測った文献は無い。
3. **SHA-256 が最小**（`hash`）——正規化テキストのダイジェスト、それが等しければ**生の答え**の
   ダイジェスト。恣意的だが、**答えそのものについてだけ**恣意的である。提示位置でも提案者でも長さでもなく、
   同じ答えなら毎回・どの機械でも同じ勝者になる。決定論的な同点解消の先例は 2605.29800 Appendix K。
   （正規化が同じでも生が違う候補は在り得る。合意群は厳密な過半数を要るので、4 体中 2 体が同文でも群には
   ならない。）
4. **どの鍵も決めなかった**（`identical`）——候補がバイト単位で同じ答えを書いたときである。id の昇順で
   最初のものを返し、**鍵の名前がそう記録される**。id は設定の属性であって提示位置ではなく、
   区別の付かない答えの間で「hash が決めた」と名乗らないための語である。

正規化して**空になる**答え（`***`、`。。。`）は、鍵を当てる前に同点集合から落とす——ただし全員がそうなら
落とさない。空の答えは同点に勝ってはならず、それを罰したはずの長さの鍵を無効化してもならない。落として
1 つになったときの記録は `length` である（空文字列は最短だからで、それが空と非空を比べられる唯一の尺度で
ある）。

`tie_break.among` と理由の文言に並ぶ id は**昇順**で書く。run が候補をどの順で提示したかに依らない同じ
一覧になる——提案者順は、そうならない。
...
本記録もそれを守る**（退けた鍵の最初の 3 つはその理由で退けた）。**長さについては転移しない**：スワップが
...
- `serve` の `cmoa` 拡張は `selection.score`、`selection.consensus`（正規化・一致の種類・一致数・母数）、
  `selection.tie_break`（鍵と同点の数。鍵は `consensus` / `length` / `hash` / `identical`）を出す。
  **候補 id は出さない**——理由の文字列も、同点の候補名を並べる末尾を落として返す。どの提案者が書いたかがトレースにしか無いのは 0011 D2 のままである。
...
- 正規化はヒューリスティックである。短い散文が同じ末尾の数を共有すると**偽の一致**になりうる
  （160 runes の上限、「数が両方にある」条件、否定の一致、桁区切りの形、指数の原文保持はそのための
  緩衝であって、証明ではない——実際、否定の安全弁はレビューで見つかった実在の失敗を塞いだものである）。
  逆に、半角カナや丸数字は畳まないので、一致を**見落とす**方には倒れる。見落としは 6 コールに落ちるだけ
  なので、安全な向きに寄せてある。
